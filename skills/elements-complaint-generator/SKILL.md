...
### 表格完整性约束（2026-08-27 用户指正）

官方要素式文书的纸面形态是**三张大表**：①主体连体大表（说明+当事人+诉讼请求+事实与理由+关联案件）②对纠纷解决方式的意愿表 ③附件原告证据清单表。部分案由的入库模板源文件（如 22-著作权）主体被拆成 4 张 w:tbl（入库/转换遗留），表间还有空段落——**渲染后必须合并回一张连体大表**，否则微调内容即引起整体布局漂移。合并方法：四张表的列实为同一主网格（22-案由主网格 [2270,611,1153,526,2097,2687]，2290=611+1153+526、4784=2097+2687、7074=后5列之和），按"行内 tc 宽度累积→主网格边界换算 gridSpan"逐行换算后并入壳表，并删除表间空段；意愿表与证据清单保持独立（其间空段保留以分页）。人工修补输出 docx 时同理：**只能在单元格内改文本，不得增删行/拆表**。

### 案由规则覆盖范围（2026-08-27 用户反映）

`_ip_specifics("经济损失")` 仅覆盖：经济损失金额、计算依据、侵权链接、律师费/取证费/差旅费行、主张诉讼费用勾选。**未覆盖**且引擎规则内也未实现的字段（部分由用户在 Word 里手补或按需扩展）：①停止侵权☑ ②著作权主体身份☑ ③作品名称/完成时间/首次发表/登记是☑/登记时间/类别 ④权属勾选（署名权☑/复制权☑/信息网络传播权☑）⑤侵权通知否☑ ⑥被诉侵权行为方式 12.其他☑ ⑦关联案件勾选 ⑧其他请求内容。22-copyright、23-trademark、27-tradesecret 三个知产案由均同此缺口。已知模板骨架（`references/skeletons/22-…md`）逐字段列出，但 `_ip_specifics` 没把它们转成 rule。后续修补方向：在 `_ip_specifics` 里加 `make_text_replace_rule` 链覆盖作品名称/完成时间/首次发表/登记时间/类别勾选；在 `make_勾选` 机制下，elements["勾选"] key 必须是骨架中"段落原文子串"（如"美术、建筑作品"，不是"作品类别"）。

### 模板几何与法院发放件不一致（2026-08-27 实测根因）

入库模板树（法〔2025〕82号源）主体表总宽 **9344 twips**，在 A4 + 1800 边距下溢出可用宽 8306，导致左右间距错乱、窄列折行；而法院实际发放的 .doc（如 250903 案北京互联网法院件）表格总宽约 **7008 twips**（左列 1702 + 右列 5305）。修复口径：渲染后全表等比缩放至 ~7008 并按需合并主体表；对比基准取法院发放件而非入库模板。另：引擎 gender/字段规则对"第三人（自然人）"块存在 occurrence 串填（被告信息被复制进第三人栏），输出后必须专项复核第三人块。
