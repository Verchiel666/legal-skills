...
一次只给模型一张联系表。不要要求模型同时审完整个总览。基础保留帧使用 `keep/drop/replace`；已丢弃候选使用 `restore/leave_discarded`。模型只填写模板中的空值，覆盖帧从 `allowed_coverage_audit_ids` 选择，理由码按结果从 `reason_codes_by_outcome` 选择，不修改 ID、哈希或选项。把 `status` 改为 `completed` 时必须填写全部组；未审完就保留 `in_progress`，不得伪装完成。

weak 模式对改变基础结果设置三重门禁：`confidence >= 0.90`、`coverage_audit_id` 指向同组基础保留帧、该组存在本地风险信号。任何一项不足时，应用器记录 `safe_noop` 并维持基础帧。模型自报置信度不能单独触发删除。
...
- weak 删除/替换未引用同组基础保留帧时不执行变更，并记录为 `safe_noop`；
- 决策缺少理由或置信度；
- 审计会产生空精选集；
