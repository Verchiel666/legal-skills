...
- **苹果日历本地库直读（双通道）**（用户授予完全磁盘访问）：Calendar.sqlitedb SQLite 直读 ≈50ms（CalendarItem⋈Calendar⋈Location；Core Data 纪元换算；hidden 过滤；跳过名单沿用）；直读失败自动回退 AppleScript；日历页状态条（未授权→"⚡去授权"一键跳系统设置页；直读→绿标）；定时刷新 10min→3min；实测 45 事件、冷请求 0.02s
- macOS 限制说明：完全磁盘访问是唯一不可程序申请的权限，无法弹窗确认——一键跳转授权页为最接近方案

## [0.8.3] - 2026-08-16

- **苹果日历定时抓取调度器**（用户反馈：应由服务内部定时更新，而非页面访问驱动）：启动预热 + 每 10 分钟后台刷新（`DASHBOARD_APPLECAL_INTERVAL` 可调，下限 120s）——抓取与页面访问完全解耦，日历页永远读缓存即显；页面触发路径仅作兜底（stale-while-revalidate 保留）

## [0.8.1] - 2026-08-16

- **日历视图增强**（用户反馈四项）："月历"更名**日历**；月视图下方新增**当月事项清单**（按日分组、日期+周几+今天标记、类型色条行、逾期/倒计时列、过去日期淡化，点击同弹预览）；**周末两列底色**暖色区分（含表头）；修复苹果事件长时段串挤爆格子（格内只显开始时间、事件条数 3→2、格子定高 104px）

## [0.8.0] - 2026-08-16

- **M7-5 苹果日历导入**：Calendar.app 本地查询（osascript AppleScript；需系统授权 自动化→ZCode→日历）合并进月历第三色 🗓（时间/全天/日历名/地点，点击弹详情）；窗口 ±30/+150 天，缓存 15 分钟；噪音日历跳过（`DASHBOARD_APPLECAL_SKIP`）；授权失败 toast 指引
- 排障沉淀：JXA whose 静默失效→AppleScript；tell 内 handler 需 `my`；§ 分隔符经 -e 损坏→tab；913 事件日历致超时→跳过清单
- 实测：38 条真实事件；9-23 同格三层（案件开庭+自记开庭+节气）

## [0.7.2] - 2026-08-16

- **焦点页近期开庭接入预览弹窗**（用户反馈：预览不应只在月历）：预览弹窗抽为通用 showEvModal（月历/焦点共用）；焦点开庭行点击即弹（详情 + 传票首页速览 + 进入案件/打开原件），带文书者行内显示 📄 标记
- 苹果日历导入开发中（等用户授权：系统设置→隐私与安全性→自动化→ZCode→日历）
