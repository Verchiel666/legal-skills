...
> **薄壳范围说明**：本文件是 `dingtalk-minutes` 技能的完整命令参考（源自钉钉官方 dws 内置 skill）。本技能为**只读薄壳**，仅使用下列**读取类**命令：`list`(mine/shared/all)、`get summary`/`get transcription`/`get keywords`/`get todos`/`get info`/`get audio`/`get batch`、`+detail`。下列**写操作超出本技能范围**，请勿在薄壳流程中调用：`update`、`replace-text`、`+replace-batch`、`upload`、`record`、`permission`、`speaker`、`mind-graph` 标签/热词管理。如需写操作，直接执行 `dws minutes <cmd>` 或切到对应写能力 skill。
