...
- 修复 intake 阻塞口径自相矛盾：`SKILL.md` §1.1 例外清单此前未列「审查目的」，与 §9.1「必问信息」冲突，导致审查目的缺失时不暂停、自行按「签约前把关」推进并选择交付强度。现将审查立场、审查目的、审查口径统一为同一张阻塞清单（§3.2），缺一即暂停，并明确立场缺失不得默认解析为「中立」「其他」。
- 修复条款边界场景强行套用三档结论的问题：关键事实缺口导致核心条款无法实质判断时（验收付款脱钩、变更费用无授权、违约金堆叠、正文引用附件未提供），不得输出「可签 / 有条件可签 / 不建议签」，改用「结论待定——信息补齐前不作签署判断」并列明缺口与补齐路径（§4.3、§9.4）。
- 修复报告生产器自报成功：`scripts/report/reporting.py` 渲染无异常即退出码 0，字段塌缩（14 处待补占位）与空法律依据（渲染为 `/`）均未被拦截。新增独立完整性复核 `check_report_integrity()`，未通过时非零退出并打印具体问题。
