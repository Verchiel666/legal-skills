...
5. **工时记录（必做）**：`log-work <短码> <时长|auto|?> <本次工作摘要> [--task] [--file 产出文书] [--type]`——**AI 协作完成的每项工作（文书起草/研究/分析/整理）都边完成边记录**：内容与关联文书/任务自动携带。**时长三态**：用户明示实际值（如"花了一下午"→4h）优先；未明示传 `auto`——**按工作类型取合理下限外扩**（文书 1h / 研究 1h / 核查 0.5h / 沟通 0.5h / 出庭 2h / 整理 0.5h；类型可从内容推断，AI 生成快但律师核查/修改的真实成本必须计入，用户拍板的计费合理冗余）；`?` 仅在用户明确不想估时使用
