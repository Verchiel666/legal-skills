...
5. **工时记录（必做）**：`log-work <短码> <时长|auto|?> <本次工作摘要> [--task] [--file 产出文书] [--type]`——**AI 协作完成的每项工作（文书起草/研究/分析/整理）都边完成边记录**：内容与关联文书/任务自动携带。**时长三态**：用户明示实际值（如"花了一下午"→4h）优先；未明示传 `auto`——**按工作类型取参考默认（出庭 2h 为底线）**（参考默认：文书·研究 1h，核查·沟通·整理 0.5h，出庭 2h 为实务底线；类型可从内容推断——AI 生成快但律师核查/修改的真实成本计入，可按实际上下调整）；`?` 仅在用户明确不想估时使用
