...
- **案件短码支持可选后缀（同日多案/一案拆分）**（用户指出：6 位日期前缀是建档日期非唯一 ID，一天接多案前缀必撞）：
  - `CASE_DIR_RE` 从 `^(\d{6})\s` 扩为 `^(\d{6}(?:-[A-Za-z0-9]+)?)\s`——目录名如 `250609-A 沉睡的法则 著作权-从方` 可被发现，短码 `250609-A` 全链路可用（show/list/add-task/validate/render）
  - case-dashboard `dashboard_server.py` 同步（发现正则 + display_name 剥离正则）
  - extract 扫描跳过 `.legacy`/`.archive-` 文件与隐藏目录（避免误扫归档件）
  - 真实数据验证：12 案件共存（存量 10 无影响 + 新增 250609-A/B），validate 双案通过

## [0.8.0] - 2026-08-16

- **`backfill-work` 工时回补命令**（用户拍板：按已有法律文书倒推完成的工作量）：
  - 扫描案件目录工作产物（02 分析/03 研究/05 证据取证/06 文书/08 法院文书审阅/09 庭审整理/10 报告），日期取文件名 YYMMDD 前缀（无则 mtime）
  - 按 日期×目录类型 分组合并（每条列出文件名，大批量如批量取证翻倍封顶）；默认 dry-run，--apply 落盘（经引擎，更新历史留痕"工时回补"）
  - 消费项目 8 案件已全量回补：146 组 +117h（合计 120h；251231 28.5h 居首）
- **下限软化**：auto 改为"参考默认"（文书·研究 1h，核查·沟通·整理 0.5h；**出庭 2h 为实务底线**），契约措辞同步
