...
- 决策：候选身份解析同时接受顶层 `version` 和 `metadata.version`，并把两者归一为合同中的 `skill.version`；如果两个位置同时存在且值不一致，按身份歧义 fail-closed。
- 边界：这只是发布字段位置兼容，不降低合同必须绑定明确版本的要求，也不把 `version` 变成所有普通 Skill 的通用必填字段；只有使用稳定性合同时才要求可解析版本。
- 影响：同一门禁可审查传统 legal-skills frontmatter 与官方 Codex 兼容 frontmatter，同时保留版本冲突的明确阻断证据。

## D-2026-07-30-01 references 采用“普通文件扁平 + 结构化资料集一层集合目录”

- 背景：原规范把 `references/` 完全扁平作为通用要求，但尼斯分类、审查指南、法规合集等资料集包含几十份按类别或章节组织的文件。全部展开到根目录会丢失资料集边界、降低导航性，也不符合通过索引按需下钻的渐进式披露目的。
- 用户确认：`trademark-assistant` 的法源子目录是专门设计，不是需要扁平化的技术债；原始材料缺失的图片资产也不作为目录结构问题处理。
- 决策：普通参考文件保持扁平；大型、内部结构稳定的资料集允许在 `references/` 下使用一层集合目录。合法集合目录必须有稳定语义、索引入口和明确读取路由；无索引、多层嵌套或为普通文件随意建目录仍可提示。
- 影响：结构审查从机械检查“是否有子目录”改为判断“是否属于结构化资料集、是否有索引和按需下钻路径”，减少误报同时保留可达性约束。

## D-2026-07-26-01 领域 checker 采用双向充分性边界

- 背景：`legal-visualization` 的元素重叠复盘显示，Skill Lint 已能识别视觉硬约束缺少 geometry/render/visual 证据，却不能也不应内置 draw.io 容器、坐标和文字排版算法。另一方面，仅要求普通正例与违规反例，仍可能让 checker 把合法容器包含误报为碰撞。
- 决策：领域算法和 checker 归目标 Skill；Skill Lint 负责验证合同、模态、阶段、正反例和多轮候选绑定证据。把通用经验抽象为双向充分性：最小违规反例防漏报，合法近似正例防误报。
- 输出边界：静态 `ISG-002` 返回规范来源证据，并明确其结论是 `NOT_VERIFIED`，不是具体领域缺陷判定。
- 影响：指令遵循审查更可操作，同时避免 Skill Lint 演变成包揽所有领域算法的代码审查器。
