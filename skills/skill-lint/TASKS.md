...
- [x] Task-016：沉淀领域 checker 的双向充分性边界，聚焦指令遵循而不越界代替领域算法审查
  - 来源：`legal-visualization` 重叠问题复盘发现，Skill Lint 能识别“视觉硬约束没有 geometry/render/visual 证据”，但不会也不应内置 draw.io 碰撞算法；同时，只有普通正例和违规反例不足以防止 checker 把合法容器包含误报为重叠。
  - 完成日期：2026-07-26
  - 结果：在指令稳定性标准中加入“最小违规反例 + 合法近似正例”双向门禁；静态 `ISG-002` 返回规范文件、行号和摘录，并明确结论是证据不足的 `NOT_VERIFIED`，不是已经发现具体领域错误。新增去具体化回归锁定证据定位和报告边界，Skill Lint 62 项测试全量通过。
