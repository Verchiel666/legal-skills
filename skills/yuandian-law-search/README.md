...
元典检索机制感知型法律研究中间层：先对案件做轻量研判，把争点和决定性事实转换为正反检索命题与查询矩阵，再按 [元典开放平台](https://open.chineselaw.com) 的向量、关键词和结构化过滤机制检索、复核并生成可追溯报告。

对于明确法条、案号或纯概念问题，可直接走简单检索；对于案件事实、争议焦点、类案对标或既有法律分析报告复盘，默认执行“轻量研判 → 命题 → 查询矩阵 → 小样本试检 → 对位复核 → 正式检索”。本 Skill 不替代完整证据审查、诉讼方案或正式法律意见。
